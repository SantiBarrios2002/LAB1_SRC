...
// Standalone 3DES implementation (ESP-IDF disables mbedtls DES by default)
...
// ---- Minimal standalone DES / 2-key 3DES-CBC implementation ----
...
static inline uint8_t desGetBit(const uint8_t *data, int bit) {
  return (data[(bit - 1) / 8] >> (7 - ((bit - 1) % 8))) & 1;
}
static inline void desSetBit(uint8_t *data, int bit, uint8_t val) {
  int idx = (bit - 1) / 8;
  int shift = 7 - ((bit - 1) % 8);
  if (val) data[idx] |= (1 << shift);
  else     data[idx] &= ~(1 << shift);
}

static void desPermute(const uint8_t *in, uint8_t *out, const uint8_t *table, int n) {
  memset(out, 0, (n + 7) / 8);
  for (int i = 0; i < n; i++)
    desSetBit(out, i + 1, desGetBit(in, table[i]));
}

static void desKeySchedule(const uint8_t *key8, uint8_t subkeys[16][6]) {
  uint8_t cd[7];
  desPermute(key8, cd, DES_PC1, 56);
  uint32_t C = ((uint32_t)cd[0] << 20) | ((uint32_t)cd[1] << 12) |
               ((uint32_t)cd[2] << 4) | (cd[3] >> 4);
  uint32_t D = ((uint32_t)(cd[3] & 0x0F) << 24) | ((uint32_t)cd[4] << 16) |
               ((uint32_t)cd[5] << 8) | cd[6];
  for (int r = 0; r < 16; r++) {
    for (int s = 0; s < DES_SHIFTS[r]; s++) {
    ...
    cd56[1] = (C >> 12) & 0xFF;
    cd56[2] = (C >> 4) & 0xFF;
    cd56[3] = ((C & 0xF) << 4) | ((D >> 24) & 0xF);
    cd56[4] = (D >> 16) & 0xFF;
    cd56[5] = (D >> 8) & 0xFF;
    cd56[6] = D & 0xFF;
    desPermute(cd56, subkeys[r], DES_PC2, 48);
  }
}

static void desProcess(const uint8_t *in8, uint8_t *out8, uint8_t subkeys[16][6], bool encrypt) {
  uint8_t ip[8];
  desPermute(in8, ip, DES_IP, 64);
  uint8_t L[4], R[4];
  memcpy(L, ip, 4);
  memcpy(R, ip + 4, 4);
  for (int r = 0; r < 16; r++) {
    int ki = encrypt ? r : (15 - r);
    uint8_t er[6];
    desPermute(R, er, DES_E, 48);
    for (int i = 0; i < 6; i++) er[i] ^= subkeys[ki][i];
    uint8_t sout[4];
    memset(sout, 0, 4);
    for (int i = 0; i < 8; i++) {
      int offset = i * 6;
      int byte0 = offset / 8, bit0 = offset % 8;
      uint8_t sixbits;
      if (bit0 <= 2) {
        sixbits = (er[byte0] >> (2 - bit0)) & 0x3F;
      } else {
        sixbits = ((er[byte0] << (bit0 - 2)) | (er[byte0 + 1] >> (10 - bit0))) & 0x3F;
      }
      int row = ((sixbits >> 4) & 2) | (sixbits & 1);
      int col = (sixbits >> 1) & 0x0F;
      uint8_t val = DES_S[i][row * 16 + col];
      sout[i / 2] |= (i % 2 == 0) ? (val << 4) : val;
    }
    uint8_t f[4];
    desPermute(sout, f, DES_P, 32);
    uint8_t newR[4];
  ...
  memcpy(rl, R, 4);
  memcpy(rl + 4, L, 4);
  desPermute(rl, out8, DES_FP, 64);
}

// 2-key 3DES CBC encrypt: E_K1, D_K2, E_K1
static void des3CbcEncrypt(uint8_t *key16, uint8_t *iv, uint8_t *data, uint8_t len) {
  uint8_t sk1[16][6], sk2[16][6];
  desKeySchedule(key16, sk1);
  desKeySchedule(key16 + 8, sk2);
  for (int off = 0; off < len; off += 8) {
    for (int i = 0; i < 8; i++) data[off + i] ^= iv[i];
    uint8_t tmp[8];
    desProcess(data + off, tmp, sk1, true);
    desProcess(tmp, data + off, sk2, false);
    desProcess(data + off, tmp, sk1, true);
    memcpy(data + off, tmp, 8);
    memcpy(iv, data + off, 8);
  }
}

// 2-key 3DES CBC decrypt: D_K1, E_K2, D_K1
static void des3CbcDecrypt(uint8_t *key16, uint8_t *iv, uint8_t *data, uint8_t len) {
  uint8_t sk1[16][6], sk2[16][6];
  desKeySchedule(key16, sk1);
  desKeySchedule(key16 + 8, sk2);
  for (int off = 0; off < len; off += 8) {
    uint8_t ct[8];
    memcpy(ct, data + off, 8);
    uint8_t tmp[8];
    desProcess(data + off, tmp, sk1, false);
    desProcess(tmp, data + off, sk2, true);
    desProcess(data + off, tmp, sk1, false);
    for (int i = 0; i < 8; i++) tmp[i] ^= iv[i];
    memcpy(data + off, tmp, 8);
    memcpy(iv, ct, 8);
  }
}
